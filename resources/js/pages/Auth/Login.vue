...
<template>
    <div class="w-full max-w-sm space-y-4">
        <div
            class="rounded-xl border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-700 dark:bg-slate-800"
        >
        ...
        >
            ...
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                One click signs you in as that role so you can see exactly what
                it can access.
            </p>
            ...
            <ul class="mt-3 space-y-1.5">
                <li v-for="account in props.demoAccounts" :key="account.email">
                    <button
                        type="button"
                        class="hover:border-primary-light flex w-full items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-2 text-left transition-colors disabled:pointer-events-none disabled:opacity-50 dark:border-slate-700 dark:bg-slate-800"
                        :disabled="form.processing"
