...
<template>
    <div class="flex w-full flex-col">
        <div class="mb-4 flex basis-full flex-col gap-4 sm:flex-wrap sm:flex-row sm:items-center sm:justify-between">
            <slot name="actions">
                <span></span>
            </slot>
            <div class="flex items-center gap-3">
                <Button variant="ghost" class="cursor-pointer" title="Click to refresh the data">
                    <RefreshCcwIcon :size="18" class="text-gray-400" @click="reloadTable(getCurrentParams())" />
                </Button>
                <div class="group relative w-full lg:w-96 print:hidden">
                    <TextInput type="search" class="pl-10 pr-2" v-model="searchTerm" size="md"
                      :placeholder="searchPlaceholder">
                        <template #leading>
                            <div class="absolute z-1 top-1/2 left-2.5 -translate-y-1/2 cursor-pointer">
                                <SearchIcon :size="18" class="text-gray-400 group-focus-within:text-global-light" />
                            </div>
                        </template>
                    </TextInput>
                </div>
        ...
        <div class="relative min-h-50">
            <div v-if="isLoading"
              class="absolute inset-0 z-10 flex items-center justify-center bg-white/50 backdrop-blur-[1px] dark:bg-slate-900/50">
                <div class="h-8 w-8 animate-spin rounded-full border-b-2 border-primary-light"></div>
            </div>

            <BaseTable :thead="props.thead" :sort="sort" :direction="direction" :sticky-last-column="props.stickyLastColumn" @sort="toggleSort">
                <slot :items="sortedItems" />

                <template #empty v-if="items?.length === 0 && !isLoading">
                    <div class="py-12 text-center">
                        <slot name="empty-state">
                            <span class="font-medium text-gray-400">No records available.</span>
                        </slot>
                    </div>
                </template>
            </BaseTable>
        ...
        <SimplePagination :meta="props.meta" :only="props.only" :route-url="''" />
    </div>
</template>
