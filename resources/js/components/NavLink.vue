...
<template>
    <Link
        ...
        :class="[
            props.class,
            {
                'link-active from-primary-light to-primary bg-linear-to-l text-white dark:from-gray-600 dark:to-gray-800':
                    isActive,
            },
        ]"
        :style="props.style"
        v-bind="$attrs"
        class="nav-menu hover:bg-primary flex items-center-safe justify-start gap-3 rounded p-2 text-sm! capitalize select-none hover:text-white min-[1440px]:text-base dark:hover:bg-gray-600"
    >
        <slot name="icon">
            <component
                class="flex max-w-10 items-center justify-start"
                :is="props.icon"
                :size="props.iconSize"
            />
        </slot>
